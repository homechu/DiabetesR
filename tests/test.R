T<-matrix(0,512,512)
T[1,512]<-1
T[-1,-512]<-diag(1,511,511)
options(digits=11)
h<-matrix(c(0.332670553,0.8068915093,0.4598775021,-0.13501102,-0.0854412739,0.0352262919,0,0),512,1)
g<-matrix(c(0,-0.0352262919,-0.0854412739,0.13501102,0.4598775021,-0.8068915093,0.332670553,0),512,1)
"%^%" <- function(mat, pow) {
...
system.time({
  bank<-matrix(NULL,512,512)
  for(j in 1:512){
    cbind(bank(j,c(T%^%(j*2-2) %*% h)))
  }
})  
